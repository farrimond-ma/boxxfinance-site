...
- <h2> How it works</h2>
- <h2> Typical scenarios</h2>  ← Open with exactly this sentence (adapt only for singular/plural if you include more than one example): "The scenario below is illustrative — a composite example reflecting the shape of deals of this kind, not a specific, identifiable transaction." Then include ONE example with CONCRETE FIGURES: loan amount, loan-to-value, term, the exit, and a plausible completion timescale — bridging completions routinely run 3-6 weeks in practice, so do not default to an implausibly fast single-digit-working-day figure to sound impressive. Frame it explicitly as hypothetical — "Consider a landlord in [town] who needs..." — NOT as something that already happened ("A landlord in [town] needed..."). Do NOT use "we arranged", "we've placed", "a deal we handled", "real numbers", "anonymised", or any other phrasing that claims this is a real, specific, previously-completed Boxx case — it isn't one, and claiming otherwise is what this instruction exists to prevent. Vary the town, figures and scenario every time — do not reuse the town or day-count from this instruction itself, and do not let the example above become a template you echo. A scenario without numbers is not acceptable.
...
- LANDLORDS ARE THE PRIMARY AUDIENCE. Where the topic allows a choice of worked example, reach for the landlord one first — a landlord buying at auction, refurbishing a rental to meet an EPC requirement, breaking a chain on a portfolio purchase, or bridging to a buy to let refinance. Name them as landlords in the copy rather than leaving the reader to infer it.
- Still target a SECOND audience alongside them: property developers and commercial investors (refurbishment, conversion, development exit), and homeowners where the scenario is genuinely residential (a chain break, a probate sale). Include at least one example from a second audience where the topic allows — but the landlord example leads.
