...
// ─── Prompt: business-audience services (asset finance, invoice finance,
// commercial mortgages, business loans, etc.) — genuinely SME/business buyers ──
function businessLocationPrompt(row, serviceUrl, chatUrl, blogLinksText) {
  return `Write a location landing page for ${row.service} in ${row.city}. Return as a single JSON object with exactly these keys:
...
- slug format: ${row.service.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and')}-${row.city.toLowerCase().replace(/\s+/g, '-')} (e.g. business-loans-leeds)
- title format: "${row.service} ${row.city}" (e.g. "Business Loans Leeds")
- metaTitle format: "${row.service} in ${row.city}" — do NOT append "| Boxx Finance" or any brand suffix (the site template adds the brand automatically)

CONTENT STRUCTURE — follow this exact order:

1. OPENING PARAGRAPH (2–3 sentences, 60–80 words)
Start with clear intent matching: "Looking for ${row.service} in ${row.city}?" or similar. Directly answer why a ${row.city} business owner would come to this page. Mention Boxx Finance and link it to ${serviceUrl} using anchor text like "${row.service.toLowerCase()} for ${row.city} businesses".
...
- "Does Boxx Finance work with businesses in ${row.city}?"
...
// ─── Prompt: bridging loans — a consumer/property audience (homeowners,
// landlords, property developers, investors), NOT "businesses"/SMEs. Bridging
// is secured against property for a short-term purpose (chain break, auction
// purchase, refurbishment, development exit) — the audience and vocabulary
// are fundamentally different from the business-finance services above, so
// this gets its own structure rather than a bolt-on note. ─────────────────────
function bridgingLocationPrompt(row, serviceUrl, chatUrl, blogLinksText) {
  const city = row.city;
...
async function generateLocationPage(row, relatedBlogs) {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: row.service === 'Bridging Finance'
          ? bridgingLocationPrompt(row, serviceUrl, chatUrl, blogLinksText)
          : businessLocationPrompt(row, serviceUrl, chatUrl, blogLinksText),
      },
    ],
  });
